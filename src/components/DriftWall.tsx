  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    ...
    const inner = (
      <span className="drift-wall__inner">
        <div className="drift-wall__flip-container">
          <div className="drift-wall__flip-front">
            <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
            <span className="drift-wall__overlay" aria-hidden="true" />
          </div>
          <div className="drift-wall__flip-back">
            <h3 className="drift-wall__back-title">{item.title}</h3>
            <p className="drift-wall__back-desc">{item.designation}</p>
          </div>
        </div>
      </span>
    );
    const commonProps = {
      className: `drift-wall__tile${activeId === id ? ' is-active' : ''}${isFlipped ? ' is-flipped' : ''}`,
      'data-tile-id': id,
      'data-col': colIndex,
      onFocus: () => activate(id, colIndex),
      onBlur: release,
      onClick: (e: React.MouseEvent) => handleTileClick(id, e)
    };
    if (item.href) {
      return (
