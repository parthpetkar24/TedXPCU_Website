...
const DepthCarousel = ({
  ...
  const notify = useCallback(
    (idx: number) => {
      setActive(idx);
      setFlippedCards({}); // Reset flips on slide change
        ...
        {data.map((item, i) => (
          ...
          >
            <div className={`depth-carousel__flip-inner ${flippedCards[i] ? 'is-flipped' : ''}`}>
              ...
              <div className="depth-carousel__flip-back">
                <div className="depth-carousel__back-content">
                  <h3 className="depth-carousel__back-title">{item.title}</h3>
                  <p className="depth-carousel__back-desc">{item.designation}</p>
                </div>
              </div>
            </div>
          </div>
