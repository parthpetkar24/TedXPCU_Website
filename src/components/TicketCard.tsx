...
var ShaderMount = class {
  ...
  textures = /* @__PURE__ */ new Map<string, WebGLTexture>();
  minPixelRatio: number;
  maxPixelCount: number;
  isSafari = isSafari();
  uniformCache: Record<string, any> = {};
  textureUnitMap = /* @__PURE__ */ new Map<string, number>();
  constructor(parentElement: any, fragmentShader: string, uniforms: any, webGlContextAttributes: WebGLContextAttributes, speed = 0, frame = 0, minPixelRatio = 2, maxPixelCount = DEFAULT_MAX_PIXEL_COUNT, mipmaps: string[] = []) {
  ...
  setTextureUniform = (uniformName: any, image: any) => {
...
function createProgram(gl: any, vertexShaderSource2: any, fragmentShaderSource: any) {
...
function hslaToRgba(hsla: any) {
  ...
  if (s === 0) {
    ...
    const hue2rgb = (p2: any, q2: any, t: any) => {
      if (t < 0) t += 1;
      if (t > 1) t -= 1;
...
var clamp = (n: any, min: any, max: any) => Math.min(Math.max(n, min), max);
...
function useMergeRefs(refs: any) {
  ...
  const refEffect = React.useCallback((instance: any) => {
    ...
    return () => {
      cleanups.forEach((refCleanup: any) => refCleanup?.());
    };
  }, refs);
  return React.useMemo(() => {
    if (refs.every((ref: any) => ref == null)) {
      return null;
    }
    return (value: any) => {
...
function setMinImageSize(img: any) {
...
async function processUniforms(uniformsProp: any) {
  const processedUniforms: Record<string, any> = {};
  ...
  Object.entries(uniformsProp).forEach(([key, value]: any) => {
...
function colorPropsAreEqual(prevProps: any, nextProps: any) {
  for (const key in prevProps) {
    if (key === "colors") {
      ...
      if (!prevProps.colors?.every((color: any, index: any) => color === nextProps.colors?.[index])) {
        return false;
      }
      continue;
    }
...
var Dithering = memo(function DitheringImpl({
  ...
  const uniforms = {
    ...
    u_shape: DitheringShapes[shape as keyof typeof DitheringShapes],
    u_type: DitheringTypes[type as keyof typeof DitheringTypes],
    u_pxSize: size,
    // Sizing uniforms
    u_fit: ShaderFitOptions[fit as keyof typeof ShaderFitOptions],
    u_scale: scale,
    u_rotation: rotation,
    u_offsetX: offsetX,
    u_offsetY: offsetY,
...
function ticketClipPath(width: any, height: any, geometry: any = TICKET_GEOMETRY) {
...
function fitScale(lines: any, opts: any) {
...
function gradientDataUrl(g: any, aspect: any) {
...
var between = (min: any, max: any, rnd: any) => min + rnd() * (max - min);
function remixTexture(prev: any, rnd = Math.random) {
...
function remixGradient(prev: any, rnd = Math.random) {
  const hue = between(8, 44, rnd);
  return {
    ...prev,
...
function remixTicketStyle(prev: any) {
  return {
...
function burst(ctx: any, at: any, opts: any) {
...
function playShutterSound({ volume = 0.35, gap = 0.045 }: any = {}) {
  const Ctor: any = typeof window !== "undefined" ? (window as any).AudioContext ?? (window as any).webkitAudioContext : void 0;
  if (!Ctor) return;
  audioCtx ??= new Ctor();
